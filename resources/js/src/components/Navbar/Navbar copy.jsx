...
const Navbar = () => {
    ...
    const { data: categoryData, isLoading } = useGetHomeCategoriesQuery(); // এই API
    ...
    const handleNavigate = (type, slug) => {
        navigate(`/shop/${type}/${slug}`);
        setShowMobileMenu(false);
        setMobileShopOpen(false);
    };

    if (isLoading) return null; // লোডিং হলে কিছু দেখাবে না
        ...
        <div ref={navRef}>
                                            ...
                                            >
                                                ...
                                                {hoveredCat === cat.id &&
                                                    cat.sub_categories?.length >
                                                        0 && (
                                                        <div
                                                            className="absolute top-0 left-full bg-cream w-[250px] p-4 rounded-lg shadow-lg z-50"
                                                            onMouseLeave={() =>
                                                                setHoveredSub(
                                                                    null
                                                                )
            ...
            <div
                ...
                className={`fixed inset-x-0 px-4 sm:px-6 lg:px-20 py-6 bg-dark2 transition-all duration-500 z-50 ${
                    showSearchbar ? "top-0 opacity-100" : "-top-full opacity-0"
                }`}
            >
